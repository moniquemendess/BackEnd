...
const autoLogin = async (req, res, next) => {
  try {
    const { email, password } = req.body; // traemos el email y password
    const userDB = await User.findOne({ email }); // buscar un usuario en el banco de datos con ese email
    if (userDB)
      if (password == userDB.password) {
        // si password for igual a passwpord del mongodb
        const token = generateToken(userDB, _id, email); // generar el token con id, email
        return res.status(200).json({
          // retorna una respuesta de 200 OK que contem  informaciones del usuario y token
          user: userDB,
          token,
        });
      } else {
